...
def ensure_weights_exist(cache_location, checkpoint_url, model_name="depth_pro.pt"):
    ...
    if not os.path.exists(checkpoint_path):
        ...
        subprocess.run(
            ["wget", "-q", "-O", checkpoint_path, checkpoint_url], check=True
        )
...
class DepthEstimator:
    def __init__(self, from_onnx=True):
        ...
        consistent_cache_location = os.path.join(
            os.path.expanduser("~"), ".depth_pro_cache", "checkpoints"
        )
        ...
        if self.from_onnx:
            ...
            onnx_model_path = ensure_weights_exist(
                consistent_cache_location, onnx_url, model_name=onnx_model_name
            )
            ...
            checkpoint_path = ensure_weights_exist(
                consistent_cache_location, checkpoint_url
            )
    ...
    def _run_onnx(self, image):
        ...
        Returns:
            tuple: (PIL.Image depth map in 'I;16' mode, focal length in pixels).
        """
        ...
        depth_map_resized = cv2.resize(
            predicted_depth, original_size, interpolation=cv2.INTER_LINEAR
        )

        depth_map_uint16 = (depth_map_resized * 1000).astype(np.uint16)
